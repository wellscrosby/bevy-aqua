...
- `Textures` (double-buffered state + published surface + pattern) and
  the render-world write node ordered after `aqua_core::AnimWavesWritten`.

## Public API

`AquaFoamPlugin`, `Textures`. Settings live on `aqua_core::OceanWaves`
(model gate) and the material uniform; this crate adds no config.

## Test alone

```
cd crates/aqua-foam && cargo test
```
