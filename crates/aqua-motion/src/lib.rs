...
//! camera marked [`aqua_core::OceanView`]. For a temporary runtime spike gate,
//! run the root showcase with `cargo run --features motion --example showcase`.
//! Set `RUST_LOG=aqua_motion=trace,wgpu_core=warn` to see the queue/draw spans.
...
fn advance_motion_epoch(
    ...
    waves: Res<aqua_core::OceanWaves>,
    bodies: Res<aqua_core::ResolvedWaterBodies>,
    ocean: Option<Res<aqua_core::Ocean>>,
    mut previous_ocean: Local<Option<aqua_core::Ocean>>,
) {
