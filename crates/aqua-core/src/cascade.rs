...
pub use aqua_geom::{LOD_COUNT, TILE_RESOLUTION};
...
pub fn add_shader(app: &mut App) {
    ...
    let handles = vec![
        server.load("embedded://aqua_core/cascade/common.wgsl"),
        server.load("embedded://aqua_core/cascade/river.wgsl"),
        server.load("embedded://aqua_core/cascade/deform.wgsl"),
        server.load("embedded://aqua_core/cascade/types.wgsl"),
    ];
    app.insert_resource(ShaderLibraries { _handles: handles });
    aqua_optics::add_shader(app);
}
